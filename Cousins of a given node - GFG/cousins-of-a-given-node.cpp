...
class Solution
{
    ...
    {
        //code here
        queue<Node*> q;vector<int> res;
        Node *cur = root;
        q.push(cur);bool breakOut=false;

        while(!q.empty() && !breakOut){
            int size = q.size();
            for(int i=0;i<size;i++){
                Node *temp = q.front();
                q.pop();
                if(temp->left ==node_to_find || temp->right==node_to_find){breakOut = true;continue;}
